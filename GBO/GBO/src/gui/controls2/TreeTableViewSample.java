...
public class TreeTableViewSample extends Application
{
    public void start(Stage stage)
    {
        ...
        p = new Person("FB", "Informatik", "sekretariat@informatik.hochschule-trier.de");
        TreeItem<Person> fbi = new TreeItem<>(p);
        p = new Person("Andreas", "Lux", "lux@hochschule-trier.de");
        fbi.getChildren().add(new TreeItem<Person>(p));
        p = new Person("Andreas", "Künkler", "kuenkler@hochschule-trier.de");
        fbi.getChildren().add(new TreeItem<Person>(p));
        p = new Person("Jörg", "Lohscheller", "lohscheller@hochschule-trier.de");
        fbi.getChildren().add(new TreeItem<Person>(p));
        p = new Person("Heinz", "Schmitz", "heinz.schmitz@hochschule-trier.de");
        rootNode.getChildren().add(fbi);

        p = new Person("FB", "Wirtschaft", "sekretariat@wirtschaft.hochschule-trier.de");
        TreeItem<Person> fbw = new TreeItem<>(p);
        p = new Person("Helge", "Rieder", "rieder@hochschule-trier.de");
        fbw.getChildren().add(new TreeItem<Person>(p));
        p = new Person("Dieter", "Steinmann", "steinmann@hochschule-trier.de");
        fbw.getChildren().add(new TreeItem<Person>(p));
        p = new Person("Wilhelm", "Steinbuß", "steinbuss@hochschule-trier.de");
        ...
        firstNameCol.setCellValueFactory(item -> item.getValue().getValue().firstNameProperty());
        ...
        lastNameCol.setCellValueFactory(item -> item.getValue().getValue().lastNameProperty());
        ...
        emailCol.setCellValueFactory(item -> item.getValue().getValue().emailProperty());
        treeTable.getColumns().add(emailCol);

        Label status = new Label();

        treeTable.getSelectionModel().selectedItemProperty().addListener((ov, oldValue, newValue) ->
        {
            ...
            status.setText("selected: " + fName + " " + lName + " (" + email + ")");
        });
